...
class DAU(nn.Module):
    ...
    def __init__(
        ...
        modules_body = [conv(n_feat, n_feat, kernel_size, bias=bias), act,
                        conv(n_feat // 2, n_feat, kernel_size, bias=bias)]  # 1)
        print('1) init : ', n_feat, kernel_size, reduction, modules_body, act)
...
class CASNet(nn.Module):
    def __init__(self, n_feat, width, kernel_size, reduction, act, bias, num_cab):
        ...
        self.up_enc3 = nn.Sequential(UpDSample(width * 8, width * 4), UpDSample(width * 4, width * 2),
                                     UpDSample(width * 2, n_feat))
        self.up_dec3 = nn.Sequential(UpDSample(width * 8, width * 4), UpDSample(width * 4, width * 2),
                                     UpDSample(width * 2, n_feat))
