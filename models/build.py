...
from .swin_transformer_cwsa import SwinTransformer as CWSwinTransformer
from .swin_transformer_parallel import  SwinTransformer as PSwinTransformer
from .swin_transformer_SE import SwinTransformer as SESwinTransformer
...
def build_model(config, is_pretrain=False):
    ...
    if model_type == 'swin':
        ...
        model = SwinMLP(img_size=config.DATA.IMG_SIZE,
                        ...
                        use_checkpoint=config.TRAIN.USE_CHECKPOINT)
    elif model_type == 'swin_cwsa':
        model = CWSwinTransformer(img_size=config.DATA.IMG_SIZE,
                                patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                in_chans=config.MODEL.SWIN.IN_CHANS,
                                num_classes=config.MODEL.NUM_CLASSES,
                                embed_dim=config.MODEL.SWIN.EMBED_DIM,
                                ...
                                use_checkpoint=config.TRAIN.USE_CHECKPOINT,
                                fused_window_process=config.FUSED_WINDOW_PROCESS,
                                chan_heads=config.MODEL.SWIN.CHAN_HEADS)

    elif model_type == 'swin_parallel':
        model = PSwinTransformer(img_size=config.DATA.IMG_SIZE,
                                patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                in_chans=config.MODEL.SWIN.IN_CHANS,
                                num_classes=config.MODEL.NUM_CLASSES,
                                embed_dim=config.MODEL.SWIN.EMBED_DIM,
    ...
    elif model_type == 'swin_se' :
        model = SESwinTransformer(img_size=config.DATA.IMG_SIZE,
                                 patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                 in_chans=config.MODEL.SWIN.IN_CHANS,
                                 num_classes=config.MODEL.NUM_CLASSES,
                                 embed_dim=config.MODEL.SWIN.EMBED_DIM,
